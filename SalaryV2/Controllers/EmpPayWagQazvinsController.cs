...
namespace SalaryV2.Controllers
{
    public class EmpPayWagQazvinsController : Controller
    {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (id != null)
                    {
                        if (id.SequrityCode != Smodel.SequrityCode)
                        {
                            ...
                            ViewBag.Contid = new SelectList(db.ContractTypes, "Contid", "Title", Contid);
                            return View();
                        }
                        else
                        {
                            var list = CorDB.vwPersonnels.Where(s => s.IdentificationCode == "4322967868").FirstOrDefault();
                            try
                            {

                                var EmployeeList = CorDB.vwPersonnels.Where(e => e.IdentificationCode == Smodel.e1 & e.AccountNo == Smodel.b2).OrderByDescending(s => s.PersonnelId).FirstOrDefault();
                                ...
                                log.Acnt = Smodel.b2;
                                log.Type = "ورود";
                                log.FullName = EmployeeList.FirstName + " " + EmployeeList.LastName;
                                log.ConType =Convert.ToString( Contid);
                                db.Logs.Add(log);
                                db.SaveChanges();
                    ...
                    if (id != null)
                    {
                        ...
                        else
                        {
                            try
                            {
                                ...
                                {
                                    ViewBag.Contid = new SelectList(db.ContractTypes, "Contid", "Title", Contid);
                                    ViewBag.Message = "  شماره حساب یا کد ملی به درستی وارد نشده است   ";
                                ...
                                log.Date = utility.PertionDate.Today();
                                log.NationCode = Smodel.e1;
                                log.FullName = Employeeinf.Name +" "+ Employeeinf.Fmly;
                                log.Acnt = Smodel.b2;
                                log.Type = "ورود";
                                log.ConType = Convert.ToString(Contid);
                                db.Logs.Add(log);
        ...
        public List<EmpVm> DetailsEm(int? YearId, int? MonthId, string id = "", string e1 = "")
        {
            ...
            if (YearId.HasValue && YearId.Value != 0 && MonthId.HasValue && MonthId.Value != 0)
            {
                int YearTitleint = int.Parse(yearT.YearTitle);
                ...
                if (Em == null)
                {
                    if (YearTitleint <= 1398)
                    {
                        ...
                        {
                            var Emo = DB.DWHEmpPayWagNews.Where(d => d.b2 == id && d.e1 == e1).AsQueryable();
                            var ggo = Convert.ToInt16(yearT.YearTitle);
                            var ddo = Convert.ToByte(MonthT.Caption);
                            Emo = Emo.Where(e => e.c4 == ggo && e.c5 == ddo);
                            if (Emo != null)
                            {
                                ViewBag.EmployeeName = DB.DWHEmpPayWagNews.Where(d => d.b2 == id && d.e1 == e1 && d.c5 == ddo && d.c4 == ggo).FirstOrDefault();
