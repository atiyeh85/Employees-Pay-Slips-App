    ...
    public class RandomNumController : ControllerBase
    {
        ...
        public ActionResult GenerateCode(GetCode GetCode)
        {
            if (ModelState.IsValid)
            {
                ...
                if (GetCode.Contid==1)
                {
                    var empProfile = CorDB.vwParties.Where(c => c.IdentificationCode == GetCode.e1 ).FirstOrDefault();
                    var empCalc = CorDB.vwCalculations.Where(d => d.AccountNo == GetCode.b2 && d.PersonnelRef == empProfile.PartyId).OrderByDescending(c => c.Year).ThenByDescending(c => c.Month).FirstOrDefault();
                    if (empProfile!=null)
                    {
                        TempData["message"] = "شماره حساب ویا کد ملی به درستی  در سیستم وارد نشده است لطفا برای رفع این مشکل  ";
                        return RedirectToAction("search", "DWHEmpPayWagNews");

                        if (empProfile.Phone!=null)
                        {
                            if (db.RandomNumbers.Where(r => r.NationCode == empProfile.IdentificationCode && r.acnt == empCalc.AccountNo).Count() > 0)
                            {
                                var InstanceRand = db.RandomNumbers.Where(r => r.NationCode == empProfile.IdentificationCode && r.acnt == empCalc.AccountNo).FirstOrDefault();
                                ...
                                InstanceRand.acnt = empCalc.AccountNo;
                            ...
                            else
                            {
                                ...
                                InsertCode.acnt = empCalc.AccountNo;
                                ...
                                db.SaveChanges();
                                utility.SmSSender.SendSmSCode(empProfile.Phone, empProfile.Name, Code.ToString());
                                TempData["message"] = "همکار گرامی پیامک حاوی کد شناسه امنیتی برای شما ارسال خواهد شد.......";
                                return RedirectToAction("search", "EmpPayWagQazvins");
                            }
                ...
                else if(GetCode.Contid ==2)
                {
                    ...
                    else if (Suser != null)
                    {
                        if (db.RandomNumbers.Where(r => r.NationCode == Suser.NationCode && r.acnt == Suser.Acnt).Count() > 0)
                        {
                            ...
                            db.Entry(InstanceRand).State = EntityState.Modified;
                            db.SaveChanges();
                            utility.SmSSender.SendSmSCode(TelMobil, Suser.Name, Code.ToString());
                            TempData["message"] = "همکار گرامی پیامک حاوی کد شناسه امنیتی برای شما ارسال خواهد شد.......";
                            return RedirectToAction("search", "EmpPayWagQazvins");
                        }
                        else
