    ...
    public class RandomNumController : ControllerBase
    {
        ...
        public ActionResult GenerateCode(GetCode GetCode)
        {
            if (ModelState.IsValid)
            {
                ...
                if (GetCode.Contid==1)
                {
                    var empProfile = CorDB.vwPersonnels.Where(c => c.IdentificationCode == GetCode.e1 && c.AccountNo == GetCode.b2).FirstOrDefault();
                    var empCalc = CorDB.vwCalculations.Where(d => d.AccountNo == GetCode.b2&& d.PersonnelRef==empProfile.PersonnelId).OrderByDescending(c=>c.Year).ThenByDescending(c=>c.Month).FirstOrDefault();
