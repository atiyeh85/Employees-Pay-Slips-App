...
namespace SalaryV2.Controllers
{
    public class vwCalculationsController : Controller
    {
        ...
        {
            var Lo = new LoanVm();
            var EmpProfile = CorDB.vwPersonnels.Where(e => e.AccountNo == id & e.IdentificationCode == e1).FirstOrDefault();
            ...
            {
                var monthid = MonthId.ToString();
                var Year = db.Years.Where(y => y.YearId == YearId).FirstOrDefault();
                var Month = db.Months.Where(y => y.Caption == monthid).FirstOrDefault();
                var mo =int.Parse(Month.Caption);
                EmpQuerable = EmpQuerable.Where(e => e.Year == YearId && e.Month == mo && e.Value != 0);
                ...
                List = CorDB.vwShreds.Where(s => s.DLCode == EmpProfile.DLCode & s.RemainedAmount > 0).ToList();
                ...
                foreach (var ite in List)
                {
                    ...
                    var ListPay = II.Where(c => c.PaymentDate != null).ToList();
                    var Coun = 0;

                    foreach (var L in ListPay)
                    {
                        DateTime thisDate = Convert.ToDateTime(L.PaymentDate.ToString());
                    ...
                    PerMonth = (int)TotalAmount / Count;
                    ...
                    ViewBag.MonthId = new SelectList(db.Months.OrderByDescending(m => m.Caption), "MonthId", "MonthTitle", LastMm.MonthId);
        ...
        public List<vwCalculation> DetailsEm(int? YearId, int? MonthId, string id = "", string e1 = "")
        {
            if (YearId.HasValue && YearId.Value != 0 && MonthId.HasValue && MonthId.Value != 0)
            {
                ...
                //ViewBag.MonthTStr = MonthT.MonthTitle;
                var YNow = db.Years.Where(y => y.YearTitle == YearNow).FirstOrDefault();
                var MNow = db.Months.Where(y => y.Caption == MonthNow).FirstOrDefault();
                var YNowid = YNow.YearId;
                var MNowid = MNow.MonthId;
                var MNowidStr = MNowid.ToString();
                var yearint = int.Parse(YNow.YearTitle);
                var monthint = int.Parse(MNow.Caption);
                var EmployeeList = CorDB.vwCalculations.Where(e => e.AccountNo == id /*& e.Year== yearint & e.Month==monthint*/ & e.Value != 0).OrderByDescending(d => d.Year).ThenByDescending(d => d.Month).ToList();
                var EmpProfile = CorDB.vwPersonnels.Where(e => e.AccountNo == id & e.IdentificationCode == e1).FirstOrDefault();
                ...
                {
                    Loan Lo = new Loan();
                    var I = CorDB.vwShredItems.Where(s => s.ShredRef == item.ShredID).OrderByDescending(s => s.PaymentDate).FirstOrDefault();
                    Lo.TotalAmount = I.Amount;
                    Lo.RemainedAmount = item.RemainedAmount;
                    Lo.Target = item.Target;

                    LoanA.Add(Lo);
                }
                ContractA.Add(co);
                ViewBag.ContractA = ContractA;
                foreach (var item in EmpQuerable.Where(m => m.ElementClass == 1 & m.ElementType != 6))
                {

                    Karkerd.Add(item);
                }
                foreach (var item in EmpQuerable.Where(m => m.ElementClass == 3))
                {
                    Sub.Add(item);
                }
                foreach (var item in EmpQuerable.Where(m => m.ElementClass == 2))
                {
                    Add.Add(item);
                }
                var Tax = EmpQuerable.Where(s => s.ElementRef == -390).FirstOrDefault();
                ...
                else
                {
                    ...
                    ViewBag.MessageMonthNull = null;
                    ViewBag.e7 = db.Months.Where(m => m.Caption == Mnt).Select(m => m.MonthTitle).FirstOrDefault();
                    EmpQuerable = EmpQuerable.Where(e => e.Year == gg && e.Month == dd).OrderByDescending(d => d.Year).ThenByDescending(d => d.Month);
                    ViewBag.EmployeeName = EmpSorted;
                    _Employees = EmpQuerable.ToList();
                    return _Employees;
                }
